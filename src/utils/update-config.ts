...
'.bash', '.ps1', '.c', '.h', '.cpp', '.hpp', '.cc', '.hh', '.rs',
'.zig', '.java', '.kt', '.kts', '.scala', '.groovy', '.cs', '.fs',
'.vb', '.swift', '.m', '.mm', '.dart', '.go', '.rb', '.rbw', '.php',
'.hs', '.ex', '.exs', '.erl', '.clj', '.sql'],
    WEB_FILES: ['.html', '.htm', '.xhtml', '.xht', '.mhtml', '.vue',
'.svelte', '.astro', '.riot', '.jsx', '.tsx', '.ejs', '.pug', '.jade',
'.handlebars', '.hbs', '.mustache', '.twig', '.nunjucks', '.njk',
'.php', '.phtml', '.jsp', '.jspx', '.asp', '.aspx', '.cshtml',
'.vbhtml', '.j2', '.jinja', '.jinja2', '.erb', '.webmanifest'],
    STYLE_FILES: ['.css', '.scss', '.sass', '.less', '.styl', '.pcss',
'.sss', '.wxss', '.acss'],
    CONFIG_FILES: ['.json', '.jsonc', '.yaml', '.yml', '.toml',
'.config.*', 'config.*', '.editorconfig', '.browserslistrc',
'browserslist', '*ignore', '.gitattributes', '.gitmodules', '.env*',
'Dockerfile*', 'docker-compose*.yml', '.github/workflows/*.yml',
'package.json', 'package-lock.json', 'yarn.lock', 'pnpm-lock.yaml',
'bun.lockb', 'composer.json', 'composer.lock', 'Cargo.toml',
'Cargo.lock', 'Gemfile', 'Gemfile.lock', '.eslintrc*',
'eslint.config.*', '.prettierrc*', 'prettier.config.*',
'.stylelintrc*', 'stylelint.config.*', 'tsconfig*.json', '.babelrc*',
'babel.config.*', 'webpack.config.*', 'vite.config.*',
'rollup.config.*', 'next.config.*', 'nuxt.config.*',
'gatsby-config.*', 'gulpfile.*', '*/settings.json',
'.vscode/**/*.json', '.idea/**/*.xml'],
    TEST_FILES: ['*.test.*', '*.spec.*', '*_test.*', '*_spec.*',
'*-test.*', 'test-*.*', 'test.*', 'test/**/*', 'test_*.py',
'*Test.php', '*Spec.scala', '__tests__/**/*', '__snapshots__/**/*',
'__mocks__/**/*', 'tests/**/*', 'specs/**/*', 'jest.config.*',
'vitest.config.*', 'playwright.config.*', 'cypress.config.*',
'karma.conf.*', 'setupTests.*', 'test-setup.*', 'test.setup.*'],
    DOC_FILES: ['README*', 'readme*', 'LICENSE*', 'license*',
'COPYING*', 'NOTICE*', 'CHANGELOG*', 'changelog*', 'HISTORY*',
'RELEASES*', '.md', '.markdown', '.mdx', '.rst', '.adoc', '.asciidoc',
'.txt', 'CONTRIBUTING*', 'contributing*', 'CODE_OF_CONDUCT*',
'code_of_conduct*', 'SECURITY*', 'SUPPORT*', 'docs/**/*', 'doc/**/*',
