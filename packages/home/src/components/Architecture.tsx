        ...
        <section id='architecture' className='px-6 py-20'>
            <div className='max-w-4xl mx-auto text-center'>
                ...
                <p className='max-w-xl mx-auto mb-12'>
                    Every non-trivial command splits into three layers, each
                    with exactly one job — the mental model is deliberately
                    React-shaped.
                </p>
                ...
                <div className='grid md:grid-cols-3 gap-6 text-left'>
                    {layers.map(layer => (
                        <div
                            key={layer.name}
                            className='pendex-card p-6 flex flex-col gap-3'
                        >
                            <span
                            ...
                            <code className='text-xs opacity-70 mt-auto'>
                                {layer.files}
                            </code>
                        </div>
                    ))}
                ...
                <p className='max-w-xl mx-auto mt-10 text-sm opacity-80'>
                    <code>ArchiveFormat.ts</code> exists because Compile and
                    Split used to each have half-knowledge of the same text
                    format — one file now owns it in both directions.
                </p>
            </div>
        </section>
    );
