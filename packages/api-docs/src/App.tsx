...
/** Prefix every internal link needs so it resolves under GitHub Pages' /pendex/ base path in production. */
const BASE = import.meta.env.BASE_URL;

/** Root of the site this app is nested under (packages/home), one level up from BASE. */
const SITE_ROOT = BASE.replace(/docs\/?$/, '');
...
const NAV_LINKS = [
    ...
    { href: `${BASE}api/`, label: 'API Reference' },
];
    ...
    return (
        <div className="min-h-screen bg-base-100 text-base-content">
            <header className="navbar border-b border-base-300 px-4 sm:px-8 sticky top-0 bg-base-100/95 backdrop-blur z-10">
                <div className="flex-1">
                    <a href="#top" className="font-display text-lg font-bold text-primary">
                        pendex<span className="text-base-content/50">/docs</span>
                    </a>
                </div>
            ...
            <main id="top">
                <section className="max-w-4xl mx-auto px-4 sm:px-8 pt-16 pb-12">
                    <BannerFrame tag="-< PENDEX >-">
                        ...
                        <p className="mt-4 text-base sm:text-lg text-base-content/80 max-w-2xl">
                            Turns an entire source tree into portable, plain-text archives —
                            ideal for AI prompts, code reviews, backups, and sharing. Compile a
                            project into banner-delimited <code className="text-accent">.txt</code> files
                            grouped by job, then reconstruct the whole tree later with{' '}
                            <code className="text-accent">px split</code>.
                        </p>
                        <div className="mt-6 flex flex-wrap gap-3">
                            <a href="#quickstart" className="btn btn-primary btn-sm">
                                Get started
                            </a>
                            <a href={`${BASE}api/`} className="btn btn-outline btn-sm">
                                Browse the API
                            </a>
                            <a
                ...
                <section id="quickstart" className="max-w-4xl mx-auto px-4 sm:px-8 py-12 border-t border-base-300">
                    <h2 className="font-display text-2xl font-bold text-secondary mb-4">Quickstart</h2>
                    <div className="mockup-code text-sm">
                        <pre data-prefix="$"><code>bun add -g pendex</code></pre>
                        <pre data-prefix="$"><code>px</code></pre>
                        <pre data-prefix=">" className="text-success"><code>? Main Menu: (Use arrow keys)</code></pre>
                        <pre data-prefix=">" className="text-base-content/60"><code>❯ Compile Codebase</code></pre>
                        <pre data-prefix=">" className="text-base-content/60"><code>  Split Archive</code></pre>
                        <pre data-prefix=">" className="text-base-content/60"><code>  Exit Program</code></pre>
                    </div>
                    <p className="mt-4 text-sm text-base-content/70">
                        Or run either step standalone, without the interactive menu:
                    </p>
                    <div className="mockup-code text-sm mt-2">
                        <pre data-prefix="$"><code>bun run packages/compile/src/Compile.ts</code></pre>
                        <pre data-prefix="$"><code>bun run packages/split/src/Split.ts</code></pre>
                    </div>
                </section>

                <section id="packages" className="max-w-4xl mx-auto px-4 sm:px-8 py-12 border-t border-base-300">
                    <h2 className="font-display text-2xl font-bold text-secondary mb-2">Packages</h2>
                    <p className="text-sm text-base-content/70 mb-6">
                        Five workspace packages, each with a single responsibility — click through
                        for the full generated API reference (types, classes, and functions,
                        straight from the source JSDoc comments).
                    </p>
                    <div className="grid sm:grid-cols-2 gap-4">
                        {PACKAGES.map((pkg) => (
                            <a
                                key={pkg.name}
                                href={`${BASE}api/${pkg.apiPath}`}
                            ...
                            >
                                <div className="card-body p-5">
                                    ...
                                    <p className="text-sm text-base-content/70">{pkg.tagline}</p>
                                </div>
                            </a>
                        ))}
                    </div>
                ...
                <section id="guide" className="max-w-4xl mx-auto px-4 sm:px-8 py-12 border-t border-base-300">
                    <h2 className="font-display text-2xl font-bold text-secondary mb-6">Guide</h2>
            ...
            <footer className="border-t border-base-300 px-4 sm:px-8 py-8 text-center text-sm text-base-content/50">
                <p>
                    Pendex is MIT-licensed. Full API reference generated from source with{' '}
                    <a href={`${BASE}api/`} className="link link-primary">
                        TypeDoc
                    </a>
                    .
                </p>
