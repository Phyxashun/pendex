//
// SINGLE RESPONSIBILITY: what a theme *means* — the DefaultTheme
// contract, the hex-map schema that TOML theme files must satisfy, and
// the builder that turns a hex map into a working DefaultTheme via
// Colors. No picocolors, no file I/O (ThemeManager owns loading), no
// chaining (useTheme owns that).
//
// BRAND_PALETTE below is the in-code copy of src/themes/pendex.toml —
...
/** The hex slots every theme file provides. Structural styles (bold, italic, ...) are behavior, not palette, and stay in code. */
...
/** Builds a working DefaultTheme from a hex map. This is the ONLY place that decides how palette slots combine with structural styles. */
