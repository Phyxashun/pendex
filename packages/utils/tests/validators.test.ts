...
describe('Validators', () => {
    describe('validateDirectory', () => {
        ...
        test('should return error for input with spaces', () => {
            expect(validateDirectory('my folder')).toBe(
                'Spaces are not allowed in directory path names.',
            );
        });

        test('should return error for invalid characters', () => {
            expect(validateDirectory('folder<name')).toBe(
    ...
    describe('validateGlobInput', () => {
        ...
        test('should return error for empty glob patterns', () => {
            expect(validateGlobInput('  ,  ')).toBe(
                'Glob lists cannot contain completely blank strings.',
            );
            expect(validateGlobInput('pattern,,pattern')).toBe(
                'Glob lists cannot contain completely blank strings.',
            );
            expect(validateGlobInput('pattern,  ')).toBe(
                'Glob lists cannot contain completely blank strings.',
            );
        });
