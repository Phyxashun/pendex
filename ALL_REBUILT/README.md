...
project into banner-delimited `.txt` files grouped by job (source,
web, style, terminal, configuration, documentation, testing, misc),
then reconstruct the whole tree later with `px split`.
...
Built on [Bun](https://bun.sh) + TypeScript, using
...
`CompileService`/`SplitService` (orchestration),
`CompileView`/`SplitView` (presentation) — each piece has exactly one
well-defined responsibility, the same way each piece of a formal
system does.
...
`compile` and `split` don't require the interactive shell — each only
needs a `Config`, which it loads itself via `ConfigManager` when run
directly:
...
each. The mental model is deliberately shaped:
...
classDiagram
    ...
    class CompileService {
        ...
        +compileJob(job, excludes, outputDir, manifest, claimed)
Promise~CompileJobOutcome~
    ...
    class SplitService {
        ...
        +splitArchiveFile(outputDir, rebuiltDir, filename)
...
`ConfigManager` (`src/components/Config.ts`) is a singleton: it parses
`config.toml` once per process, merges `runtime.config.json` on top if
present, and hands back the same `Config` object to every command.
...
refactor above — that injects or strips a
